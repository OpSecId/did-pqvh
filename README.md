...
- **`POST /dids`** — create (201); fails with **409** if that `state.id` already exists. Response body is **`{ "logEntry": { ... }, "bootstrap": { ... }? }`**. You may send **`{"options":{},"parameters":{},"state":{}}`**: the server registers a new ML-DSA signing key (same shape as **`POST /keys`**), sets **`parameters.preRotationKeys`** to that key’s **`preRotationKey`**, generates **`options.apiKey`**, and defaults **`state`** to a minimal DID document with **`id`: `did:pqvh:{SCID}`**. **`bootstrap`** echoes generated **`apiKey`** and/or key material when the server filled those in (omit **`bootstrap`** when you supplied both yourself). If you already use **`POST /keys`**, supply **`preRotationKeys`** and **`options.apiKey`** as before.
...
Minimal create (server-generated signing key + API key; read secrets from **`bootstrap`** in the JSON response):
