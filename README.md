...
- **`GET /{scid}`** — read stored signed entry. **`{scid}`** may be the **bare base58 SCID** (e.g. `QmWty8to1v573wR3ZSj88FScJFY6JaVijGuJAA8UugrhoX`) or the full **`did:pqvh:`** + that string (one path segment; percent-encode colons if your HTTP client requires it). Registered after reserved paths (`/health`, `/keys`, `/resolve`, `/credentials`, etc.).
...
- **`GET /resolve?did={did}`** — resolve by full **`did:pqvh:…`** or bare SCID (local store lookup in this prototype).
...
For a SCID entry created via this API, **`GET /{scid}`** returns a did:webvh-style signed entry including **`versionId`**, **`versionTime`**, **`state`**, **`parameters`**, and **`proof`**.
