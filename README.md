...
- **`POST /`** — create (201); fails with **409** if that `state.id` already exists. Response body is **`{ "logEntry": { … } }`**. The response also includes header **`X-Scid-Auth-Secret`**: a per-DID secret the client must store and send on **`PUT`** / **`DELETE`** for that DID (same header name). You may send **`{"options":{},"parameters":{},"state":{}}`**: the server registers a new ML-DSA signing key (same shape as **`POST /keys`**), sets **`parameters.preRotationKeys`** to that key’s **`preRotationKey`**, and defaults **`state`** to a minimal DID document with **`id`: `did:pqvh:{SCID}`**. If you already use **`POST /keys`**, supply **`preRotationKeys`** as before. Use **HTTPS** in real deployments so the secret is not exposed on the wire; reverse proxies may log request headers—configure them not to log this header.
